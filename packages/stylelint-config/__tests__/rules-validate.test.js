...
describe('test/rules-validate.test.js', () => {
    it('Validate default', async () => {
        ...
        const result = await stylelint.lint({
            configFile: path.join(__dirname, '../index.js'), files: filePaths, fix: false,
        })
    ...
    it('Validate sass', async () => {
        ...
        const result = await stylelint.lint({
            configFile: path.join(__dirname, '../index.js'), files: filePaths, fix: true
        })
        if (result && result.errored) {
            const filesResult = JSON.parse(result.output || '[]') || []
            filesResult.forEach(fileResult => {
    ...
    it('Validate css-module', async () => {
        const filePaths = [path.join(__dirname, './fixtures/css-module-test.scss')]
        const result = await stylelint.lint({
            configFile: path.join(__dirname, '../index.js'), files: filePaths, fix: true
        })
        if (result && result.errored) {
